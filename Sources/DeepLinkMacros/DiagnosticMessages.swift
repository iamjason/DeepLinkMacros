...
enum DiagnosticError: Error, CustomStringConvertible {
  ...
  var description: String {
    ...
    case .unsupportedParameterType(let type):
      return "Unsupported parameter type '\(type)'. Supported types: String, Int, Bool, Double, [String], [Int], [Double], [Bool], and RawRepresentable enums"
    ...
    case .unsupportedArrayElementType(let type):
      return "Unsupported array element type '\(type)'. Supported array element types: String, Int, Double, Bool. Arrays of enums are not supported."
    }
  }
}
